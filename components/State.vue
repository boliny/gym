<template>
  <section ref="statsSection" class="bg-beige gap-6 sm:gap-8 grid-cols-auto-fit py-16 flex justify-center space-x-10 text-center">
    <div v-for="(stat, index) in stats" :key="index" class="stat">
      <h2 class="text-5xl font-bold">
        {{ Math.round(stat.current) }}<span v-if="stat.suffix">{{ stat.suffix }}</span>
      </h2>
      <p class="text-sm font-medium uppercase text-gray-600">{{ stat.label }}</p>
    </div>
  </section>
</template>
