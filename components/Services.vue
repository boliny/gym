<template>
  <div class="bg-light-blue py-10">
    ...
    <div class="relative max-w-5xl mx-auto">
      ...
      <!-- إخفاء الأسهم عند الأجهزة الصغيرة -->
      <button
        v-if="showNavigation"
        @click="prevSlide"
      ...
      <!-- إخفاء pagination عند الشاشات الصغيرة -->
      <div v-if="showPagination" class="pagination flex justify-center mt-4">
        <div
          v-for="(_, index) in testimonials.length"
          :key="index"
          @click="moveToSlide(index)"
          :class="['dot', { active: currentSlide === index }]"
          class="w-4 h-4 mx-1 bg-gray-300 rounded-full cursor-pointer transition-all hover:bg-gray-500"
        ></div>
      </div>
    </div>
  </div>
  ...
  setup() {
    ...
    const updateSliderSettings = () => {
      if (window.innerWidth < 768) {
        ...
        showPagination.value = true; // إظهار pagination
      }
    ...
    onMounted(() => {
      updateSliderSettings(); // تحديث الإعدادات عند التحميل
    ...
    const prevSlide = () =>
      slider.value.moveToIdx(slider.value.track.details.rel - 1, true);
    const nextSlide = () =>
      slider.value.moveToIdx(slider.value.track.details.rel + 1, true);
